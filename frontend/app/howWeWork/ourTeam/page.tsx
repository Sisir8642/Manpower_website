...
function TeamMemberSkeleton() {
  return (
    <div className="bg-white border border-blue-200 rounded-3xl p-2 flex flex-col items-center animate-pulse">
      <div className="w-44 h-64 rounded-2xl bg-gray-200 mb-6" />
...
const TeamPage = () => {
  const { data: teamData, isLoading } = useQuery<TeamMember[] | { results: TeamMember[] }>({
    queryKey: ["team-members"],
    queryFn: async () => {
      const res = await fetch(`${baseUrl}/api/Team_member/`);
  ...
  return (
    <div>
      ...
      <section
        ...
        style={{ backgroundImage: "url('/images/banner/OurTeam.jpeg')" }}
      >
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div>
            ...
            <div className={`grid ${gridCols} gap-6`}>
              {isLoading
                ? Array.from({ length: 5 }).map((_, i) => <TeamMemberSkeleton key={i} />)
                : teamMembers.length === 0
                ? (
                  <p className="text-gray-400 col-span-5 text-center py-10">
                      ...
                      <div className="flex-grow flex flex-col justify-between w-full">
                        <div>
                          ...
                          <p className="text-red-600 font-bold text-xs mt-1.5 leading-snug tracking-tight">
                            {member.position}  {/* Django field is 'position' not 'role' */}
                          </p>
                        </div>
          ...
          <FadeIn delay={400} className="mt-24 border-t border-blue-200/60 pt-10 text-center">
            ...
            <p className="text-slate-700 font-medium max-w-4xl mx-auto leading-relaxed text-xs sm:text-sm italic">
              "We believe recruitment is more than connecting people with employment—it is about building futures,
              empowering individuals, and creating meaningful global opportunities through ethical and responsible
              practices. Driven by integrity, professionalism, and a commitment to excellence, our team delivers
              seamless support throughout every phase of recruitment, including pre-employment, on-employment,
              and post-employment services for both candidates and employers."
            </p>
