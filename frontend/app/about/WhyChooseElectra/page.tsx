...
export default function WhyChooseElectra() {
  return (
    <div>
      {/* Hero Section */}
      <section
        className="relative overflow-hidden text-white bg-cover bg-center min-h-[40vh] sm:min-h-[50vh] md:min-h-[60vh]"
        style={{ backgroundImage: "url('/images/banner/WhyChoose.jpg')" }}
      >
        <div className="absolute inset-0 bg-black/30"></div>
      ...
      {/* Features Section */}
      <section className="bg-[#E1F1E6] text-slate-900 font-sans min-h-screen flex flex-col justify-center overflow-x-hidden py-12 sm:py-16 md:py-20">
        ...
        >
          ...
          <p className="text-slate-800 font-semibold mt-3 sm:mt-4 max-w-xl mx-auto text-sm sm:text-base leading-relaxed px-4">
            Ethical recruitment built safely on structural trust, compliance regulatory standards, and global migration management rules.
          </p>
        </motion.div>
            ...
            {features.map((item, i) => {
              ...
              return (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: i * 0.04, duration: 0.5, ease: "easeOut" }}
                  className={`group relative p-4 sm:p-5 md:p-6 lg:p-7 rounded-2xl bg-blue-50/70 border border-blue-200 shadow-sm shadow-blue-900/5 transition-all duration-300 hover:bg-white hover:-translate-y-1 flex flex-col items-start justify-start ${item.hoverBorder}`}
                >
                  <div
                    className={`w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 flex items-center justify-center rounded-xl mb-3 sm:mb-4 md:mb-5 transition-transform duration-300 group-hover:scale-105 border shadow-inner ${item.colorClass}`}
                  >
                    <Icon className="text-base sm:text-lg md:text-xl" />
                  </div>

                  <h3 className="font-black text-slate-950 text-sm sm:text-[14px] md:text-[15px] mb-1.5 sm:mb-2 tracking-tight group-hover:text-slate-900 transition-colors duration-300 leading-snug">
                    {item.title}
                  </h3>
                  ...
                  <div className={`absolute bottom-0 left-1/2 w-0 h-[2px] sm:h-[3px] rounded-b-2xl group-hover:w-full group-hover:left-0 transition-all duration-300 ${item.lineColor}`} />
                </motion.div>
              );
            })}
          </div>
